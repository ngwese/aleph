# Porting apps/mix to a block-based mix module

Almost all of the work is a new Blackfin module. `apps/mix` only needs a new embedded LDR; it already loads DSP as an opaque SPI boot image and does not care about frame vs block.

## How it works today

`apps/mix` embeds the frame module from `modules/mix` as `.ldr` bytes and boots it once in `app_launch` (`src/app_mix.c`):

- The LDR bytes live in `src/aleph-mix.ldr.inc` and `src/aleph-mix.ldr_size.inc`.
- Launch path: `bfin_load_buf(ldrData, ldrSize)` → `bfin_wait_ready()` → `bfin_enable()`.
- Control uses hardcoded param indices that must match `modules/mix/params.h` (`src/ctl.h`, `src/ctl.c`).

There is no SD module picker and no Makefile dependency on `modules/mix`.

There is currently **no** `modules_block/mix/`. The only in-tree block module is `modules_block/rawsc/`. No app currently launches a block module.
...
Create something like `modules_block/mix/`, modeled on `modules_block/rawsc/` plus the logic from `modules/mix/`:

| Piece | Change |
|--------|--------|
| Makefile | Point at `bfin_lib_block` (`include ../../bfin_lib_block/bfin_lib_block.mk`), `module_name = mix` |
...
| Init | Set `gModuleData->name` (rawsc does; frame mix does not) |
...
mix = sum of (*in)[ch][j] * adcVal[ch];
(*out)[0..3][j] = mix;
```

Call `filter_1p_lo_next` **per sample** inside the block if you want the same slew behavior as frame mix. Updating once per block would make slews roughly `MODULE_BLOCKSIZE` times slower.

Closest template: copy structure from `modules_block/rawsc/`, DSP behavior from `modules/mix/mix.c`.

### 2. CV gap in `bfin_lib_block` (blocker for a faithful port)

Frame mix drives CV DACs via `cv.h` / `cv_update()` from `bfin_lib`. **`bfin_lib_block` has no `cv.c` / `cv.h`.** It does initialize SPORT1 for the AD5686 (`serial.c`), but there is no `cv_update` API.

Options:

1. **Port CV into `bfin_lib_block`** — bring over `cv.c` / `cv.h` from `bfin_lib`, then keep CV params.
2. **Drop CV** from the block mix — remove `eParam_cv*` / `eParam_cvSlew*` and the matching `ctl_param_change` calls in `src/ctl.c`.

The mix app only zeros CV at init; the UI never touches CV. Dropping CV is viable for this app if only mixing matters.

### 3. Rebuild and re-embed the LDR into `apps/mix`

After building the block module:

1. Build `modules_block/mix` → `mix.ldr`
2. Run `bintool` on that LDR → new `aleph-mix.ldr.inc` + `aleph-mix.ldr_size.inc` (see `utils/bintool/README.md`)
3. Replace the copies under `apps/mix/src/`
4. Rebuild the AVR32 mix app
...
- `apps/mix` Makefile / `config.mk` / load path
...
1. Add `modules_block/mix/` (Makefile → `bfin_lib_block`, `MODULE_BLOCKSIZE`, block process loop).
...
4. Build LDR → `bintool` → refresh `src/aleph-mix.ldr*.inc` → rebuild app.
